...
from board import (CELL_X, CELL_O, RESULT_X_WINS, RESULT_O_WINS, RESULT_DRAW, play_game, Board)
...
class TTTNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.first_layer = nn.Linear(INPUT_SIZE, 36)
        self.second_layer = nn.Linear(36, 36)
        self.output_layer = nn.Linear(36, OUTPUT_SIZE)
    def forward(self, board):
        board = self.first_layer(board)
        board = torch.relu(board)
        board = self.second_layer(board)
        board = torch.relu(board)
        board = self.output_layer(board)
...
def get_game_result_value(player, board):
    if is_win(player, board):
        return (WIN_VALUE, "WON")
    if is_loss(player, board):
        return (LOSS_VALUE, "LOST")
    if is_draw(board):
        return (DRAW_VALUE, "DRAW")


def update_training_gameover(net_context, move_history, player, final_board, discount_factor):
    [game_result_reward, result] = get_game_result_value(player, final_board)
...
def play_training_game(net_context, move_history, player, x_strategy, o_strategy, discount_factor):
    board = play_game(x_strategy, o_strategy)

    update_training_gameover(net_context, move_history, player, board, discount_factor)


def choose_move_index(board, model, epsilon):
    if epsilon > 0:
        random_value_from_0_to_1 = np.random.uniform(0, 1)
...
def play_training_games(net_context, player, total_games, discount_factor, epsilon, x_strategies, o_strategies):
    if x_strategies:
    ...
    for game in range(total_games):
        ...
        play_training_game(net_context, move_history, player, x_strategy_to_use, o_strategy_to_use, discount_factor)
...
def play_training_games_x(net_context, total_games=2, discount_factor=1.0, epsilon=0.7, o_strategies=None):
    play_training_games(net_context, CELL_X, total_games, discount_factor, epsilon, None, o_strategies)


def play_training_games_o(net_context, total_games=2, discount_factor=1.0, epsilon=0.7, x_strategies=None):
